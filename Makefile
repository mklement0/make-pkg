...
update-man: 
	@read -r cliName cliPath < <(json -f package.json bin | json -Ma key value | head -n 1) || { [[ '$(MAKECMDGOALS)' == 'update-man' ]] && echo "WARNING: Nothing to do; no CLI is defined for this package." >&2; exit 0; }; \
	 ...
	 mkdir -p doc man; \
	 "$$cliPath" --man-source > doc/"$$cliName".md || { echo "Please make sure that '$$cliName --man-source' outputs the Markdown-formatted man-page source." | fold -s >&2; exit 1; }; \
	 ...
	 [[ '$(MAKECMDGOALS)' == 'update-man' ]] && echo "-- 'doc/$$cliName.md' and 'man/$$cliName.1' updated; to view the latter as a man page, run: man man/$$cliName.1" || :
...
toggle-man:
	@isOn=$$([[ `json -f package.json net_same2u.make_pkg.manOn` == 'true' ]] && printf 1 || printf 0); \
	 ...
	 if (( isOn )); then \
	 	 ...
	 	 echo "-- Man-page creation is now ON."; echo "Note that '$$cliName --man-source' must output the man-page source in Markdown format for this to work." | fold -s; \
	 fi
